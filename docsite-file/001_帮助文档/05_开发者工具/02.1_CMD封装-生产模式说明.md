### ** 添加记录：**
**20190528**
　　1. 不建议在xml中配置resource，resource配置均移动至TaskStage中去
**20190106**
　　1. 增加语法糖"(>)"
**20180425**
...
&nbsp;
**复杂属性配置：**
<span style='font-size:18px;font-weight:bold'>&lt;resource&gt;</span>：本xml所需的cpu和内存资源，可以是数字，也可以是公式。(过时，不建议使用) 
　　有以下属性：
　　**cpu**：本xml所需要的cpu数量。
　　**mem**：本xml所需要的内存大小。
　　最简单的例子如下：
...
　　其中`value="ILLUMINACLIP:[%s]:%s:%s:%s"`中，用"[]"来标记文件
	**(>)**
	默认我们用">"来定位标准输出流。但是有时候我们希望把输出流直接压缩为.gz，而又不想先输出文件再压缩。这时候可以写成
	`<script id="prefix" type="Output" param="(>)" value="%s.gz"/>`
	注意把管道输出写成"(>)"这种形式，这样输出的结果就是压缩好的gz文件。
	**//，/**
