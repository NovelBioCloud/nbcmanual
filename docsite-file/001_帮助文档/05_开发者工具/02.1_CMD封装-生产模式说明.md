...
　　2. 过时文件夹config
　　3. 添加文件夹bin
　　4. 修改taskdatabase_path为database_path，修改tasks_root_path为tasks_path，添加task_bin_path
**20180329**
...
　　在scriptmodule中，每个task拥有一个以该task名字命名的文件夹，其内部可以有五个文件夹，分别为 Prepare， Run， Summary， conifg， bin。其中前三个文件夹中放置task三个阶段所需运行的xml文件。config文件夹主要用于自动跳过等配置(待补充)，bin文件夹中建议放置运行所使用到的各种脚本和所使用到的软件(可无这两个文件夹)。
```
		例子：
